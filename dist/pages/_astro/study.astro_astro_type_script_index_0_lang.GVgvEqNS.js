document.addEventListener("DOMContentLoaded",function(){const d=document.getElementById("studyMat"),f=document.querySelector("#file"),l=document.getElementById("fileUploadContainer"),T=document.getElementById("fileUploadText"),h=document.getElementById("fileList"),c=document.getElementById("fileError"),m=document.querySelector('button[type="submit"]'),x=document.querySelector('button[type="reset"]'),I=document.getElementById("uploadProgress"),M=document.getElementById("progressBar"),S=document.getElementById("progressText");let i=[],p=!1;const D=[{id:"subject",errorId:"subjectError",message:"Please select a subject."},{id:"semester",errorId:"semesterError",message:"Please select a semester."},{id:"title",errorId:"titleError",message:"Please enter a title."},{id:"file",errorId:"fileError",message:"Please upload a file."},{id:"fileDescription",errorId:"fileDescriptionError",message:"Please enter a file description."}];function F(){l.classList.add("uploading"),I.style.display="block";let e=0;const t=setInterval(()=>{e+=Math.random()*15,e>100&&(e=100),M.style.width=e+"%",S.textContent=Math.round(e)+"%",e>=100&&(clearInterval(t),setTimeout(()=>{I.style.display="none",l.classList.remove("uploading"),M.style.width="0%",S.textContent="0%"},500))},200)}function q(){let e=document.getElementById("submissionModal");e||(document.body.insertAdjacentHTML("beforeend",`
        ...
        `,new bootstrap.Modal(t).show()}function O(){return{subject:document.getElementById("subject")?.value||"",semester:document.getElementById("semester")?.value||"",title:document.getElementById("title")?.value||"",description:document.getElementById("fileDescription")?.value||"",files:i.map(e=>({name:e.name,size:(e.size/1024/1024).toFixed(2)+" MB",type:e.type}))}}function P(){const e=Date.now().toString(36),t=Math.random().toString(36).substr(2,5);return`SM-${e}-${t}`.toUpperCase()}function N(){if(p)return;p=!0,m.disabled=!0,m.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status"></span>Submitting...',q();const e=new FormData(d);e.delete("file"),i.forEach((t,s)=>{e.append("files[]",t)}),e.append("submission_id",P()),e.append("submission_timestamp",new Date().toISOString()),e.append("file_count",i.length.toString()),setTimeout(()=>{console.log("Form Data to be submitted:");for(let[t,s]of e.entries())console.log(t,s);p=!1,m.disabled=!1,m.innerHTML="SUBMIT"},3e3)}function k(){i=[],f.value="",b([]),l.classList.remove("error","has-files","uploading"),I.style.display="none",M.style.width="0%",S.textContent="0%",c&&(c.textContent=""),d.reset(),document.querySelectorAll(".is-invalid, .is-valid").forEach(e=>{e.classList.remove("is-invalid","is-valid")}),document.querySelectorAll(".error-message").forEach(e=>{e.textContent="",e.style.display="none"}),m.disabled=!1,m.innerHTML='<i class="bi bi-send me-1"></i>Submit Material',p=!1}function w(){const e=new DataTransfer;i.forEach(t=>e.items.add(t)),f.files=e.files}function Y(e,t){const s=new bootstrap.Modal(document.getElementById("filePreviewModal")),a=document.getElementById("filePreviewModalLabel"),n=document.getElementById("filePreviewContent"),o=document.getElementById("removeFileBtn");a.innerHTML='<i class="bi bi-file-earmark-check me-2"></i>File Confirmation & Preview';const g=K(e.type,e.name),B=e.size<=100*1024*1024?'<span class="badge bg-success">Size OK</span>':'<span class="badge bg-danger">Size Too Large</span>',u=_(e.name)?'<span class="badge bg-success">Format Supported</span>':'<span class="badge bg-warning">Format Not Supported</span>',se=`
        ...
        `;let y="";if(e.type==="application/pdf"){const r=URL.createObjectURL(e);y=`
            ...
            `;return}l.classList.add("has-files"),T.innerHTML=`<strong>${e.length} file(s) selected</strong>`,h.innerHTML=e.map((t,s)=>`
