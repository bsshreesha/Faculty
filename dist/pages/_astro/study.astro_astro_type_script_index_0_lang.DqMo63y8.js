document.addEventListener("DOMContentLoaded",function(){const r=document.getElementById("studyMat"),m=document.querySelector("#file"),n=document.getElementById("fileUploadContainer"),M=document.getElementById("fileUploadText"),b=document.getElementById("fileList"),v=document.getElementById("fileError"),g=document.querySelector('button[type="submit"]'),y=document.querySelector('button[type="reset"]'),S=document.getElementById("uploadProgress"),B=document.getElementById("progressBar"),T=document.getElementById("progressText");let i=[],h=!1;const F=[{id:"subject",errorId:"subjectError",message:"Please select a subject."},{id:"semester",errorId:"semesterError",message:"Please select a semester."},{id:"title",errorId:"titleError",message:"Please enter a title."},{id:"file",errorId:"fileError",message:"Please upload a file."},{id:"fileDescription",errorId:"fileDescriptionError",message:"Please enter a file description."}];function D(){n.classList.add("uploading"),S.style.display="block";let e=0;const t=setInterval(()=>{e+=Math.random()*15,e>100&&(e=100),B.style.width=e+"%",T.textContent=Math.round(e)+"%",e>=100&&(clearInterval(t),setTimeout(()=>{S.style.display="none",n.classList.remove("uploading"),B.style.width="0%",T.textContent="0%"},500))},200)}function A(){let e=document.getElementById("submissionModal");e||(document.body.insertAdjacentHTML("beforeend",`
        ...
        `,new bootstrap.Modal(t).show()}function q(){return{subject:document.getElementById("subject")?.value||"",semester:document.getElementById("semester")?.value||"",title:document.getElementById("title")?.value||"",description:document.getElementById("fileDescription")?.value||"",files:i.map(e=>({name:e.name,size:(e.size/1024/1024).toFixed(2)+" MB",type:e.type}))}}function P(){const e=Date.now().toString(36),t=Math.random().toString(36).substr(2,5);return`SM-${e}-${t}`.toUpperCase()}function O(){if(h)return;h=!0,g.disabled=!0,g.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status"></span>Submitting...',A();const e=new FormData(r);e.delete("file"),i.forEach((t,s)=>{e.append("files[]",t)}),e.append("submission_id",P()),e.append("submission_timestamp",new Date().toISOString()),e.append("file_count",i.length.toString()),setTimeout(()=>{console.log("Form Data to be submitted:");for(let[t,s]of e.entries())console.log(t,s);h=!1,g.disabled=!1,g.innerHTML="SUBMIT"},3e3)}function x(){const e=new DataTransfer;i.forEach(t=>e.items.add(t)),m.files=e.files}function N(e,t){const s=new bootstrap.Modal(document.getElementById("filePreviewModal")),a=document.getElementById("filePreviewModalLabel"),l=document.getElementById("filePreviewContent"),o=document.getElementById("removeFileBtn");a.innerHTML='<i class="bi bi-file-earmark-check me-2"></i>File Confirmation & Preview';const f=Y(e.type,e.name),I=e.size<=100*1024*1024?'<span class="badge bg-success">Size OK</span>':'<span class="badge bg-danger">Size Too Large</span>',c=V(e.name)?'<span class="badge bg-success">Format Supported</span>':'<span class="badge bg-warning">Format Not Supported</span>',te=`
