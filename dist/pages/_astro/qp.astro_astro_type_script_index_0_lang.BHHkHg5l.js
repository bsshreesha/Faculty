document.addEventListener("DOMContentLoaded",function(){const c=document.getElementById("studyMat"),m=document.querySelector("#file"),n=document.getElementById("fileUploadContainer"),I=document.getElementById("fileUploadText"),b=document.getElementById("fileList"),v=document.getElementById("fileError"),g=document.querySelector('button[type="submit"]');document.querySelector('button[type="reset"]');const M=document.getElementById("uploadProgress"),S=document.getElementById("progressBar"),B=document.getElementById("progressText");let i=[],y=!1;const T=[{id:"subject",errorId:"subjectError",message:"Please select a subject."},{id:"semester",errorId:"semesterError",message:"Please select a semester."},{id:"title",errorId:"titleError",message:"Please enter a title."},{id:"file",errorId:"fileError",message:"Please upload a file."},{id:"fileDescription",errorId:"fileDescriptionError",message:"Please enter a file description."}];function P(){n.classList.add("uploading"),M.style.display="block";let e=0;const t=setInterval(()=>{e+=Math.random()*15,e>100&&(e=100),S.style.width=e+"%",B.textContent=Math.round(e)+"%",e>=100&&(clearInterval(t),setTimeout(()=>{M.style.display="none",n.classList.remove("uploading"),S.style.width="0%",B.textContent="0%"},500))},200)}function H(){let e=document.getElementById("submissionModal");e||(document.body.insertAdjacentHTML("beforeend",`
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            <div class="d-flex justify-content-between">
                                ...
                                <span class="fw-semibold font-monospace">${F()}</span>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="d-flex justify-content-between">
        ...
        `,new bootstrap.Modal(t).show()}function O(){return{subject:document.getElementById("subject")?.value||"",semester:document.getElementById("semester")?.value||"",title:document.getElementById("title")?.value||"",description:document.getElementById("fileDescription")?.value||"",files:i.map(e=>({name:e.name,size:(e.size/1024/1024).toFixed(2)+" MB",type:e.type}))}}function F(){const e=Date.now().toString(36),t=Math.random().toString(36).substr(2,5);return`PYQ-${e}-${t}`.toUpperCase()}function R(){if(y)return;y=!0,g.disabled=!0,g.innerHTML='<span class="spinner-border spinner-border-sm me-2" role="status"></span>Submitting...',H();const e=new FormData(c);e.delete("file"),i.forEach((t,s)=>{e.append("files[]",t)}),e.append("submission_id",F()),e.append("submission_timestamp",new Date().toISOString()),e.append("file_count",i.length.toString()),setTimeout(()=>{console.log("Form Data to be submitted:");for(let[t,s]of e.entries())console.log(t,s);y=!1,g.disabled=!1,g.innerHTML="SUBMIT"},3e3)}function h(){const e=new DataTransfer;i.forEach(t=>e.items.add(t)),m.files=e.files}function q(e,t){const s=new bootstrap.Modal(document.getElementById("filePreviewModal")),a=document.getElementById("filePreviewModalLabel"),l=document.getElementById("filePreviewContent"),o=document.getElementById("removeFileBtn");a.innerHTML='<i class="bi bi-file-earmark-check me-2"></i>File Confirmation & Preview';const p=Y(e.type,e.name),L=e.size<=100*1024*1024?'<span class="badge bg-success">Size OK</span>':'<span class="badge bg-danger">Size Too Large</span>',r=V(e.name)?'<span class="badge bg-success">Format Supported</span>':'<span class="badge bg-warning">Format Not Supported</span>',ee=`
                    ...
                    <div class="card-body">
                        <div class="alert alert-info border-0 mb-0">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <h6 class="alert-heading mb-1">${N(e.name)}</h6>
